...
sub call_method {
    ...
    my $ctx = Bio::KBase::KmerAnnotationByFigfam::ServiceContext->new(client_ip => $self->_plack_req->address);
    
    my $args = $data->{arguments};

    # Service KmerAnnotationByFigfam does not require authentication.
...
sub get_method
{
    ...
    {
	$self->exception('NoSuchMethod',
			 "'$method' is not a valid method in service KmerAnnotationByFigfam.");
    }
...
package Bio::KBase::KmerAnnotationByFigfam::ServiceContext;

use strict;

=head1 NAME

Bio::KBase::KmerAnnotationByFigfam::ServiceContext
